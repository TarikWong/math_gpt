...
def transfer_test_data_to_template(test_data):
    ...
    for one_data in test_data:
        one_data.pop("conversations")
        one_data["conversations"] = [{"value": json.dumps(one_data, ensure_ascii=False, default=obj_to_dict)},
                                     {"value": "-"}]
...
def extract_thought(target_generation):
    # list of string where the string is between <thought> and </thought>
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate responses using trained models")
    parser.add_argument("--model_name", type=str, help="Name of the model to use", default="llama")
    parser.add_argument("--model_path", type=str, help="Path to the model checkpoint",
                        default="/mnt/pfs/zitao_team/wangtuo/llm_team/source/training/sft/train_scripts/models/Llama_2_70b_hf_sft_question_step/checkpoint-10")
    parser.add_argument("--test_file_path", type=str, help="Path to the test data file",
                        default="/mnt/pfs/zitao_team/big_model/wangtuo_data/train_data/source2_sample8_test_gpt.json")
    parser.add_argument(
        "--result_save_dir",
        type=str,
        default="results",
    ...
    parser.add_argument("--test_num", type=int, default=10, help="Number of test items to use")
    ...
    # Set device
    device = "cpu" if not torch.cuda.is_available() else "cuda"
    print("args.cuda_visible_devices: ", args.cuda_visible_devices)
    ...
    for idx in [1, 2, 3]:
        ...
        print("inference result is: {}".format(one_res))
        print("answer is: {}".format(test_data[idx]['conversations'][1]['value']))

    print("Done! The inference task has succeeded with Python!")
